"""
Generate STGCN config files for Phase 1a adjacency experiment.

2 datasets x 4 adj methods x 3 seeds = 24 configs
"""
...
def generate_configs():
    ...
    for dataset in DATASETS:
        ...
        for adj_method in ADJ_METHODS:
            for seed in SEEDS:
                filename = f"STGCN_{dataset}_{adj_method}_s{seed}.py"
                filepath = os.path.join(output_dir, filename)
                content = STGCN_TEMPLATE.format(
                    dataset=dataset,
                    adj_method=adj_method,
                    seed=seed,
    ...
    print(f"Generated {len(generated)} STGCN config files in {output_dir}/")
