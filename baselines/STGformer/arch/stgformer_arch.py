...
from gating.gate import NodePairGateMVP

class Mlp(nn.Module):
    """
    Drop-in replacement for timm.models.vision_transformer.Mlp
    """
...
class GraphPropagate(nn.Module):
    ...
    def forward(self, x, graph):
        ...
        for k in range(1, self.Ks):
            if graph.dim() == 4:
                # graph: [B, T, N, N]
                x_k = torch.einsum("bthi,btij->bthj", graph, x_k.clone())
            else:
                # graph: [T, N, N]
...
class SelfAttentionLayer(nn.Module):
    def __init__(
        ...
        self.attn = nn.ModuleList(
            [
                FastAttentionLayer(model_dim, num_heads, qkv_bias, kernel=kernel)
...
class STGformer(nn.Module):
    def __init__(
        ...
        self.gate_mvp = NodePairGateMVP(T=in_steps, C=input_dim, emb_dim=32, hidden=64) if use_gate else None
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, train: bool, **kwargs):
        """feedforward function of STGformer.

        Args:
            history_data (torch.Tensor): historical data with shape [B, L, N, C]
            future_data (torch.Tensor): future data with shape [B, L, N, C] (not used in STGformer)
            batch_seen (int): current batch index
        ...
        """
        # history_data: (batch_size, in_steps, num_nodes, input_dim + aux)
        x_raw = history_data
        batch_size = x_raw.shape[0]

        if self.tod_embedding_dim > 0:
            tod = x_raw[..., -2] * self.steps_per_day  # time of day feature is second to last
        if self.dow_embedding_dim > 0:
            dow = x_raw[..., -1] * 7  # day of week feature is 
        x = x_raw[..., : self.input_dim]
        ...
        if self.gate_mvp is not None:
            gate_A = self.gate_mvp(x_raw[..., : self.input_dim])  # [B, N, N]
        else:
            gate_A = None
        ...
        if gate_A is not None:
            graph = gate_A.unsqueeze(1) * graph.unsqueeze(0)  # [B, T, N, N]
