...
class SelfAttentionLayer(nn.Module):
    """
    Self-attention layer that supports both B T N D and B N D input shapes.
    
    For B T N D inputs:
    - dim=1: temporal attention (over T dimension)
    - dim=2: spatial attention (over N dimension)
    
    For B N D inputs:
    - dim=-2: attention over N dimension (default)
    
    The underlying AttentionLayer only uses the last two dimensions (-2, -1),
    so it works with any number of leading dimensions.
    """
    ...
    def forward(self, x, dim=-2):
        """
        Args:
            x: Input tensor of shape (B, T, N, D) or (B, N, D)
            dim: Dimension to apply attention over
                - For (B, T, N, D): dim=1 (temporal) or dim=2 (spatial)
                - For (B, N, D): dim=-2 (default, over N dimension)
        Returns:
...
class CrossAttentionLayer(nn.Module):
    """
    Cross attention layer that supports both B T N D and B N D input shapes.
    ...
    def forward(self, query, key, value):
        """
        Args:
            query: [B, T, num_nodes_in_group, model_dim] or [B, num_nodes_in_group, model_dim]
                - spatial attention output
            key: [B, T, K, model_dim] or [B, K, model_dim]
                - client attention output
            value: [B, T, K, model_dim] or [B, K, model_dim]
                - client attention output
            
        Returns:
            out: Same shape as query - cross attention output
        """
        # Cross attention 수행
        residual = query
...
class NodeSummaryLayer(nn.Module):
    def __init__(self, model_dim, subgraph_adj, num_tokens=1):
        super().__init__()
        self.model_dim = model_dim
        # [수정 1] register_buffer를 사용해야 to(device) 호출 시 자동으로 이동됨
    ...
    def forward(self, x):
        ...
        # 1. Graph Smoothing (이웃 정보 집계)
        # (N, N) @ (B, T, N, D) -> (B, T, N, D)
        # PyTorch broadcasting이 자동으로 처리함
        x = self.subgraph_adj @ x 
        
        # 2. Node Reduction (N -> K)
        # Linear는 마지막 차원에 적용되므로 transpose 필요
        x = x.transpose(-2, -1)  # [B, T, D, N]
        x = self.node_reduction(x)  # [B, T, D, K]
        x = x.transpose(-2, -1)  # [B, T, K, D]
...
class AttentionPoolLayer(nn.Module):
    def __init__(self, model_dim, num_tokens=4, num_heads=8, dropout=0.0):
        ...
        # [수정 1] 학습 가능한 토큰 초기화 개선
        self.tokens = nn.Parameter(torch.empty(1, num_tokens, model_dim))
        nn.init.xavier_uniform_(self.tokens) # 혹은 nn.init.normal_(self.tokens, std=0.02)
        ...
        self.norm = nn.LayerNorm(model_dim) # [선택] 안정성을 위해 Norm 추가 권장
    ...
    def forward(self, x):
        ...
        # (B*T, K, D) 형태로 Query 확장
        q = self.tokens.expand(B*T, -1, -1)
        
        # Attention (Q=Learnable Tokens, K=V=Local Nodes)
        # out: (B*T, K, D)
        ...
        # [선택] Residual은 차원이 달라서(N!=K) 불가능하지만 Norm은 가능
        out = self.norm(out) 

        return out.reshape(B, T, self.num_tokens, D)


class TokenExpansionLayer(nn.Module):
    def __init__(self, model_dim, num_heads=8, dropout=0.0):
        ...
        self.proj = nn.Linear(model_dim, model_dim)
        self.dropout = nn.Dropout(dropout)
        #self.norm = nn.LayerNorm(model_dim) # [선택] 안정성용

    def forward(self, client_features, server_tokens):
        """
        Args:
            client_features: [B, T, N, D] (Query) - "나(노드)한테 필요한 정보 내놔"
            server_tokens:   [B, T, K, D] (Key/Value) - "여기 요약된 전역 정보 있어"
        """
        # Cross attention
        # out: [B, T, N, D]
        out = self.attn(client_features, server_tokens, server_tokens)
...
class Mlp(nn.Module):
    """  from timm https://github.com/huggingface/pytorch-image-models/blob/main/timm/layers/mlp.py
    ...
    def __init__(
        ...
        bias = tuple([bias, bias])
        drop_probs = tuple([drop, drop])
...
def mlp(in_dim: int, hidden: list, out_dim: int, act=nn.ReLU, bias: bool = True) -> nn.Sequential:
    """Simple MLP builder (Sequential 반환, dropout 없음)"""
    layers = []
    d = in_dim
    for h in hidden:
