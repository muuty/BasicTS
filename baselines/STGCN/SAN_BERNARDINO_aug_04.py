...
from contrastive.augmentation import AugmentedGSO
...
# Edge drop augmentation settings
EDGE_DROP_RATE = 0.4  # Probability of dropping each edge during training

# Wrap GSO with AugmentedGSO for edge drop augmentation
augmented_gso = AugmentedGSO(original_gso=adj_mx, edge_drop_rate=EDGE_DROP_RATE)
...
CFG.TRAIN.CKPT_SAVE_DIR = os.path.join(
    'checkpoints',
    MODEL_ARCH.__name__,
    '_'.join([DATA_NAME, "EDGE_DROP", str(EDGE_DROP_RATE), str(CFG.TRAIN.NUM_EPOCHS), str(INPUT_LEN), str(OUTPUT_LEN)])
)
