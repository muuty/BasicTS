"""
STGCN with Denoising Encoder v2 + Noise Augmentation.

Improved combo:
- Encoder v2: residual connection + hidden_dim=64 + stronger noise pretraining
- Frozen pretrained DenoisingEncoder cleans input (all 5ch -> select flow for STGCN)
- Training-time noise augmentation builds model robustness

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/STGCN/SAN_BERNARDINO_denoising_v2_noisy.py', gpus='1')"
"""
import os
import sys
import torch
...
CFG = EasyDict()
CFG.DESCRIPTION = 'STGCN + Denoising Encoder v2 (residual+h64) + Noise Augmentation'
CFG.GPU_NUM = 1
...
############################## Model Configuration ##############################
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'STGCN_denoising_v2_noisy'
