...
NUM_CLIENTS = 10
NUM_TOKENS = 0  # 0으로 설정하면 Independent Learning 모드 (pooling & server 건너뛰기)
POOLING_METHOD = 'simple'
...
# 서버 모델 파라미터
SERVER_MODEL_PARAMS = {
    "num_nodes": NUM_NODES,  # 전체 노드 수 (참조용)
    "hidden_dim": 128,       # Client의 hidden_size와 반드시 일치해야 함
    "num_heads": 4,          # Multi-head Attention 헤드 수
    "num_layers": 1,         # Self-Attention Layer 깊이
    "feed_forward_dim": 256, # Attention 내부 FFN의 hidden dimension (보통 hidden * 2 or 4)
    "dropout": 0.1,
}
