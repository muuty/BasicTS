...
# ─────────────────────────────────────────────────────────────
# Server Spatial 파라미터 (Global Attention on Tokens)
# NOTE: `baselines/HierSplit/arch/model.py`의 `HierSplitServer`는 backbone과 무관하게
# Transformer self-attention을 사용합니다. 따라서 여기서는 model_dim/num_heads/dropout만 설정합니다.
...
MODEL_PARAM = {
    ...
    # STGCN은 encoder 내부에 spatial 처리가 포함되어 있으므로 client spatial은 Identity로 둡니다.
    "spatial_cls": torch.nn.Identity,
