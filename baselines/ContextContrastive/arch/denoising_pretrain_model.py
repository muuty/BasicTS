"""
Denoising Pre-training Model.

Injects synthetic sensor noise (gaussian, bias, stuck, drift) into clean data
and trains the DenoisingEncoder to reconstruct the clean signal.

Key losses:
  - L_recon: MAE on corrupted nodes (primary denoising objective)
  - L_passthrough: MAE on clean nodes (prevents over-denoising)

The encoder learns to detect and correct noisy inputs while passing clean data through.
"""
...
class DenoisingPretrainModel(nn.Module):
    """
    Denoising autoencoder for traffic sensor noise.

    Forward pass:
    1. Inject random noise into a fraction of nodes (physical channels only)
    2. Encode the noisy input via DenoisingEncoder
    3. Reconstruct clean signal via reconstruction head
    4. Return both for loss computation
    """
    ...
    def __init__(
        ...
        self.noise_types = noise_types or ['gaussian', 'bias', 'stuck', 'drift', 'dead']
    ...
    def _inject_noise(self, x: torch.Tensor):
        ...
        if noise_type == 'gaussian':
            ...
            # Simulate complete sensor failure: all physical channels → 0
            for ch in self.physical_channels:
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Args:
            history_data: [B, T, N, C] where C=5

        Returns:
            dict with prediction, recon_pred, recon_target, noise_mask
        """
        ...
        # Encode (encoder denoises physical channels, passes tod/dow through)
        ...
        # Extract physical channels from encoder output as reconstruction
        recon = z[..., self.physical_channels]  # [B, T, N, n_physical]
        recon_target = history_data[..., self.physical_channels]  # [B, T, N, n_physical]
        ...
        if reliability is not None:
            result['reliability'] = reliability  # [B, T, N, 1]
        return result
