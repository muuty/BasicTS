"""
Pre-training model for InputSpilloverCorrectorV2.

Same denoising objective as v1, but returns reliability for direct r supervision.
"""
...
class InputCorrectorPretrainModelV2(nn.Module):
    ...
    def __init__(
        ...
        self.encoder = InputSpilloverCorrectorV2(
            input_dim=input_dim,
            d_model=d_model,
    ...
    def forward(self, history_data, future_data=None, batch_seen=None, epoch=None, train=True, **kwargs):
        ...
        # Encode with reliability for r supervision loss
        z, r = self.encoder.encode(x_noisy, return_reliability=True)  # r: [B, N, 1]
        ...
        return {
            ...
            'reliability': r.squeeze(-1),   # [B, N] for BCE loss
        }
