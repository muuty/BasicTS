"""Residual Correction with Representation (3feat encoder) - MSE Loss

Same as residual_repr_3feat.py but with MSE loss to penalize large errors more.
MSE gradient is proportional to error magnitude → worst nodes get stronger signal.

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/residual_repr_3feat_mse.py', gpus='0')"
"""
import os, sys
...
CFG = EasyDict()
CFG.DESCRIPTION = 'Residual Correction with Representation (3feat) - MSE Loss'
CFG.GPU_NUM = 1
...
CFG.MODEL = EasyDict({
    'NAME': 'Residual_repr_3feat_mse_3mo',
    'ARCH': ResidualMLP,
    'PARAM': MODEL_PARAM,
...
CFG.TRAIN = EasyDict({
    ...
    'OPTIM': EasyDict({
        'TYPE': 'Adam',
        'PARAM': {'lr': 0.001, 'weight_decay': 1e-5},
    }),
