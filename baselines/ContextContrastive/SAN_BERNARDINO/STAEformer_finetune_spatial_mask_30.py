"""
Ablation Study: Fine-tuning with Spatial Masking 30% Pre-trained Encoder

Evaluates the encoder pre-trained with 30% spatial (node) masking.
Compare with 15% version to find optimal mask ratio.

Prerequisites:
    Run pre-training first: pretrain_spatial_mask_30.py

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/STAEformer_finetune_spatial_mask_30.py', gpus='0')"
"""
import os
import sys
...
############################## Encoder Configuration (SPATIAL MASKING 30%) ##############################
ENCODER = {
    'type': 'TransformerEncoder',
    'source': 'pretrained',
    'ckpt_path': f'checkpoints/ContextContrastive_spatial_mask30_3mo/xtraffic_SAN_BERNARDINO_30_{INPUT_LEN}_{OUTPUT_LEN}/*/Pretrain_SpatialMask30_best_val_MAE.pt',
    'input_dim': 3,
    'd_model': D_MODEL,
...
CFG = EasyDict()
CFG.DESCRIPTION = 'Ablation: Fine-tune with Spatial Masking 30% Pre-trained Encoder'
CFG.GPU_NUM = 1
...
############################## Model Configuration ##############################
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'STAEformer_SpatialMask30'
