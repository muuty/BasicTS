"""
Ablation Study: Fine-tuning with Spatial Masking Pre-trained Encoder

Evaluates the encoder pre-trained with spatial (node) masking augmentation
on the downstream traffic prediction task.

Prerequisites:
    Run pre-training first: pretrain_spatial_mask.py

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/STAEformer_finetune_spatial_mask.py', gpus='0')"
"""
import os
import sys
...
############################## Encoder Configuration (SPATIAL MASKING) ##############################
ENCODER = {
...
CFG = EasyDict()
CFG.DESCRIPTION = 'Ablation: Fine-tune with Spatial Masking Pre-trained Encoder'
CFG.GPU_NUM = 1
...
############################## Model Configuration ##############################
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'STAEformer_SpatialMask'
