"""
Ablation Study: Fine-tuning with Feature Masking Pre-trained Encoder

Evaluates the encoder pre-trained with feature masking augmentation
on the downstream traffic prediction task.

Prerequisites:
    Run pre-training first: pretrain_feature_mask.py

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/STAEformer_finetune_feature_mask.py', gpus='1')"
"""
import os
import sys
...
############################## Encoder Configuration (FEATURE MASKING) ##############################
ENCODER = {
    'type': 'TransformerEncoder',
    'source': 'pretrained',
    'ckpt_path': f'checkpoints/ContextContrastive_feature_mask_3mo/xtraffic_SAN_BERNARDINO_30_{INPUT_LEN}_{OUTPUT_LEN}/*/Pretrain_FeatureMask_best_val_MAE.pt',
    'input_dim': 3,
    'd_model': D_MODEL,
...
CFG = EasyDict()
CFG.DESCRIPTION = 'Ablation: Fine-tune with Feature Masking Pre-trained Encoder'
CFG.GPU_NUM = 1
...
############################## Model Configuration ##############################
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'STAEformer_FeatureMask'
...
CFG.TRAIN.CKPT_SAVE_DIR = os.path.join(
    'checkpoints',
    'ContextContrastive_finetune_feature_mask_3mo',
