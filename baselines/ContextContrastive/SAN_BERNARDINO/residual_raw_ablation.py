"""Residual Correction with Raw Input (Ablation - no encoder)

Frozen backbone (STAEformer) + Trainable ResidualMLP from raw features.
No encoder used. This is the ablation to show representation matters.

Final prediction: y_backbone + delta_y

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/residual_raw_ablation.py', gpus='0')"
"""
import os, sys
...
from baselines.ContextContrastive.arch import ResidualMLP
...
BACKBONE_CFG = {
    ...
    'feature_indices': [0, 1, 2],  # speed, tod, dow from FORWARD_FEATURES
}

############################## No ENCODER — raw input ablation ##############################

############################## ResidualMLP (trainable) ##############################
# Input: raw features (speed, tod, dow) = 3
MODEL_PARAM = {
    "input_dim": 3,  # raw: speed, tod, dow
    "in_steps": INPUT_LEN,
    "out_steps": OUTPUT_LEN,
    "hidden_dim": 128,
    "dropout": 0.1,
...
############################## Config ##############################
CFG = EasyDict()
CFG.DESCRIPTION = 'Residual Correction with Raw Input (Ablation)'
CFG.GPU_NUM = 1
CFG.RUNNER = ResidualLearningRunner
CFG.BACKBONE = BACKBONE_CFG
# No CFG.ENCODER — runner will use raw input mode
...
CFG.MODEL = EasyDict({
    'NAME': 'Residual_raw_ablation_3mo',
    'ARCH': ResidualMLP,
    'PARAM': MODEL_PARAM,
    'FORWARD_FEATURES': [0, 3, 4],  # speed, tod, dow
    'TARGET_FEATURES': [0],
})
