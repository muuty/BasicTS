...
Ablation: disable_cross_attn=True (Stage 1 + Stage 3 only, no cross-attention).

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/pretrain_input_corrector_nodead_no_crossattn.py', gpus='1')"
"""
import os
import sys
...
CFG = EasyDict()
CFG.DESCRIPTION = 'InputSpilloverCorrector pretrain NODEAD - ablation: no cross-attn'
CFG.GPU_NUM = 1
...
############################## Model Configuration ##############################
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'InputCorrectorPretrain_NODEAD_NoCrossAttn'
