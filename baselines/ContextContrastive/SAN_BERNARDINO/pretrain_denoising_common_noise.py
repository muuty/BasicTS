"""
Cross-noise generalization: Train with COMMON noise only (gaussian, bias, drift).
Test generalization to unseen structural noise (stuck, dead).

For RQ4: Does the denoising encoder generalize to unseen noise types?
"""
import os
import sys
...
MODEL_PARAM = {
    ...
    "noise_types": ["gaussian", "bias", "drift"],  # common noise only
}
NUM_EPOCHS = 30

CFG = EasyDict()
CFG.DESCRIPTION = 'Denoising pretrain: common noise only (gaussian/bias/drift)'
CFG.GPU_NUM = 1
...
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'DenoisingPretrain_CommonNoise'
