"""
Test-Time Adaptation: Ensemble Mode

Uses multiple forward passes with dropout for prediction diversity.
Simple, fast, no gradient computation at test time.

TTA Mode: ensemble
- ENSEMBLE_RUNS: 5 forward passes
- ENSEMBLE_DROPOUT: 0.1 dropout for diversity

Based on: spatial_mask freeze (best performing)

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/STAEformer_TTA_ensemble.py', gpus='0')"
"""
import os
import sys
...
TTA = {
    'ENABLED': True,
    'MODE': 'ensemble',           # Use ensemble TTA

    # Ensemble settings
    'ENSEMBLE_RUNS': 5,           # Number of forward passes
    'ENSEMBLE_DROPOUT': 0.1,      # Dropout for diversity
}

############################## General Configuration ##############################
CFG = EasyDict()
CFG.DESCRIPTION = 'TTA Ensemble: Spatial Mask Freeze + Ensemble at Test Time'
CFG.GPU_NUM = 1
...
############################## Model Configuration ##############################
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'STAEformer_TTA_Ensemble'
...
CFG.TRAIN.CKPT_SAVE_DIR = os.path.join(
    'checkpoints',
    'ContextContrastive_TTA_ensemble_3mo',
