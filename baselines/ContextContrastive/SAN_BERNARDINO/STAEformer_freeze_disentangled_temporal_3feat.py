"""STAEformer with FROZEN Disentangled + Temporal-Aware Encoder (3-feature)

Uses encoder pretrained with 3 features (flow, tod, dow):
- z_context: Temporal-Aware Contrastive Loss
- z_self: Reconstruction Loss (all 3 features)

z_context + z_self -> concat -> downstream STAEformer

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/STAEformer_freeze_disentangled_temporal_3feat.py', gpus='0')"
"""
import os, sys
...
CFG = EasyDict()
CFG.DESCRIPTION = 'STAEformer + FROZEN Disentangled Temporal (3feat: flow+tod+dow)'
CFG.GPU_NUM = 1
...
CFG.MODEL = EasyDict({
    'NAME': 'STAEformer_freeze_disentangled_temporal_3feat_3mo',
    'ARCH': STAEformer,
    'PARAM': MODEL_PARAM,
