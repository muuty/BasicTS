"""
Ablation Study: Frozen Encoder with Baseline Temporal Contrast Pre-trained

Evaluates the encoder pre-trained with baseline temporal contrast - FROZEN (no fine-tuning).
Compare with fine-tuned version to measure the quality of learned representations.

Prerequisites:
    Run pre-training first: pretrain_contrastive.py

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/STAEformer_freeze_baseline.py', gpus='0')"
"""
import os
import sys
...
############################## Encoder Configuration (BASELINE TEMPORAL CONTRAST - FROZEN) ##############################
ENCODER = {
    'type': 'TransformerEncoder',
    'source': 'pretrained',
    'ckpt_path': f'checkpoints/ContextContrastive_pretrain_3mo/xtraffic_SAN_BERNARDINO_30_{INPUT_LEN}_{OUTPUT_LEN}/*/ContrastivePretrainModel_*.pt',
    'input_dim': 3,
    'd_model': D_MODEL,
...
CFG = EasyDict()
CFG.DESCRIPTION = 'Ablation: Frozen Encoder with Baseline Temporal Contrast'
CFG.GPU_NUM = 1
...
############################## Model Configuration ##############################
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'STAEformer_Baseline_Freeze'
...
CFG.TRAIN.CKPT_SAVE_DIR = os.path.join(
    'checkpoints',
    'ContextContrastive_freeze_baseline_3mo',
