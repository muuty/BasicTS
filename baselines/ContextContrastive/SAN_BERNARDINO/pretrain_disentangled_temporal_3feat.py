"""
Disentangled + Temporal-Aware Pretraining (3-feature version)

Input: 3 features (flow, tod, dow)
Reconstruction: 3 features (flow, tod, dow) - all features
...
Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/pretrain_disentangled_temporal_3feat.py', gpus='0')"
"""
import os
import sys
...
# 3-feature input: flow(0), tod(3), dow(4) -> becomes indices 0, 1, 2
# 1-feature reconstruction: only flow (tod/dow are deterministic, no need to reconstruct)
MODEL_PARAM = {
    'input_dim': 3,        # flow, tod, dow
    'recon_dim': 1,        # Only reconstruct flow (first feature)
    'd_model': D_MODEL,
    ...
    # tod/dow are at indices 1, 2 in 3-feature input
    'tod_idx': 1,
    'dow_idx': 2,
...
CFG = EasyDict()
CFG.DESCRIPTION = 'Disentangled+Temporal Pretrain (3feat): flow+tod+dow input, flow-only reconstructed'
CFG.GPU_NUM = 1
...
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'DisentangledTemporal_3feat_3mo'
