"""Q1 Ablation: z_context only (5feat encoder, contrastive branch only)

Tests whether z_context alone is sufficient, or disentanglement adds value.

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/STAEformer_freeze_5feat_context_only.py', gpus='0')"
"""
import os, sys
...
CFG_ENCODER = {
    ...
    'nhead': 4,
    'dropout': 0.1,
    'fusion': 'context_only',  # Only z_context branch
    'include_tod_dow': True,
    'tod_idx': 3,
    'dow_idx': 4,
}

# context_only outputs d_model = 64
MODEL_PARAM = {
    "num_nodes": NUM_NODES,
    "in_steps": INPUT_LEN,
    "out_steps": OUTPUT_LEN,
    "input_dim": D_MODEL + 2,  # z_context(64) + tod + dow
    "output_dim": 1,
    "steps_per_day": 288,
...
CFG = EasyDict()
CFG.DESCRIPTION = 'Q1 Ablation: z_context only (5feat pretrained)'
CFG.GPU_NUM = 1
...
CFG.MODEL = EasyDict({
    'NAME': 'STAEformer_5feat_context_only_3mo',
    'ARCH': STAEformer,
    'PARAM': MODEL_PARAM,
