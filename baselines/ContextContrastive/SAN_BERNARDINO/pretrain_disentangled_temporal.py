"""
Disentangled + Temporal-Aware Pretraining

Combines the best of both approaches:
- z_context: Trained with Temporal-Aware Contrastive Loss (smart negative sampling)
- z_self: Trained with Reconstruction Loss (direct supervision)

This should give:
- Better overall MAE (from temporal-aware negatives on z_context)
- Better robustness (from self-representation z_self)

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/pretrain_disentangled_temporal.py', gpus='0')"
"""
import os
import sys
...
CFG = EasyDict()
CFG.DESCRIPTION = 'Disentangled + Temporal-Aware Pretrain: Contrastive(z_context) + Reconstruction(z_self)'
CFG.GPU_NUM = 1
...
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'DisentangledTemporal_3mo'
