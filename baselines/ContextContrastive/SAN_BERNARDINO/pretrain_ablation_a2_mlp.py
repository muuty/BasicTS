"""
Ablation A2: Simple MLP Encoder (no temporal, no spatial processing).
Per-node, per-timestep MLP (3→64→64→3) + residual.
Tests whether spatio-temporal inductive bias is necessary.
"""
import os
import sys
...
from ..arch.simple_mlp_encoder import SimpleMLPPretrainModel
...
MODEL_ARCH = SimpleMLPPretrainModel
NUM_NODES = 893
...
CFG = EasyDict()
CFG.DESCRIPTION = 'Ablation A2: Simple MLP encoder (no temporal/spatial)'
CFG.GPU_NUM = 1
...
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'AblationA2_SimpleMLP'
