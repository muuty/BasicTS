"""Gated Residual Correction with Raw Input (Ablation)

Node-adaptive gating with raw input (no encoder). Ablation for gated approach.

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/gated_residual_raw_ablation.py', gpus='0')"
"""
import os, sys
...
CFG = EasyDict()
CFG.DESCRIPTION = 'Gated Residual Correction with Raw Input (Ablation)'
CFG.GPU_NUM = 1
...
CFG.MODEL = EasyDict({
    'NAME': 'GatedResidual_raw_ablation_3mo',
