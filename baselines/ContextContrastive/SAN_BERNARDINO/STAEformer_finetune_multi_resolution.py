"""
Fine-tuning with Multi-Resolution Pre-trained Encoder

Evaluates the encoder pre-trained with multi-resolution contrast (5min vs 15min)
on the downstream traffic prediction task.

Prerequisites:
    Run pre-training first: pretrain_multi_resolution.py

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/STAEformer_finetune_multi_resolution.py', gpus='0')"
"""
import os
import sys
...
############################## Encoder Configuration (Unified) ##############################
ENCODER = {
    'type': 'TransformerEncoder',
    'source': 'pretrained',
    'freeze': False,  # Fine-tune the encoder
    'ckpt_path': f'checkpoints/ContextContrastive_multi_resolution_3mo/xtraffic_SAN_BERNARDINO_30_{INPUT_LEN}_{OUTPUT_LEN}/*/Pretrain_MultiResolution_best_val_MAE.pt',
    'lr': ENCODER_LR,
...
CFG = EasyDict()
CFG.DESCRIPTION = 'Fine-tune Multi-Resolution Pre-trained Encoder'
CFG.GPU_NUM = 1
...
############################## Model Configuration ##############################
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'STAEformer_MultiResolution'
...
CFG.TRAIN.CKPT_SAVE_DIR = os.path.join(
    'checkpoints',
    'ContextContrastive_finetune_multi_resolution_3mo',
