"""Residual Correction with Raw Input (Ablation) - CVaR Loss

CVaR loss only optimizes the worst 10% of samples per batch.

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/residual_raw_ablation_cvar.py', gpus='0')"
"""
import os, sys
...
CFG = EasyDict()
CFG.DESCRIPTION = 'Residual Correction with Raw Input (Ablation) - CVaR Loss (worst 10%)'
CFG.GPU_NUM = 1
...
CFG.MODEL = EasyDict({
    'NAME': 'Residual_raw_ablation_cvar_3mo',
    'ARCH': ResidualMLP,
    'PARAM': MODEL_PARAM,
