"""Residual Correction with Representation (3feat encoder)

Frozen backbone (STAEformer) + Frozen encoder (DisentangledEncoder 3feat)
+ Trainable ResidualMLP that predicts correction from representation.

Final prediction: y_backbone + delta_y

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/residual_repr_3feat.py', gpus='0')"
"""
import os, sys
...
# Input: concat(z_context, z_self) = d_model * 2 = 128
MODEL_PARAM = {
    "input_dim": D_MODEL * 2,  # 128: concat of z_context + z_self
    "in_steps": INPUT_LEN,
    "out_steps": OUTPUT_LEN,
    "hidden_dim": 128,
    "dropout": 0.1,
...
############################## Config ##############################
CFG = EasyDict()
CFG.DESCRIPTION = 'Residual Correction with Representation (3feat encoder)'
CFG.GPU_NUM = 1
