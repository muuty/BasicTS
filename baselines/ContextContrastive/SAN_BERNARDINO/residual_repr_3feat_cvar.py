"""Residual Correction with Representation (3feat encoder) - CVaR Loss

CVaR loss only optimizes the worst 10% of samples per batch.

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/residual_repr_3feat_cvar.py', gpus='0')"
"""
import os, sys
...
from baselines.ContextContrastive.loss.cvar_loss import masked_cvar_mae
...
CFG = EasyDict()
CFG.DESCRIPTION = 'Residual Correction with Representation (3feat) - CVaR Loss (worst 10%)'
CFG.GPU_NUM = 1
...
CFG.MODEL = EasyDict({
    'NAME': 'Residual_repr_3feat_cvar_3mo',
    'ARCH': ResidualMLP,
    'PARAM': MODEL_PARAM,
