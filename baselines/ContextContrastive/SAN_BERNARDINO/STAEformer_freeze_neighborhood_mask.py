"""
Ablation Study: Frozen Encoder with Neighborhood Masking Pre-trained

Evaluates the encoder pre-trained with neighborhood masking - FROZEN (no fine-tuning).
Compare with fine-tuned version to measure the quality of learned representations.

Prerequisites:
    Run pre-training first: pretrain_neighborhood_mask.py

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/STAEformer_freeze_neighborhood_mask.py', gpus='0')"
"""
import os
import sys
...
############################## Encoder Configuration (NEIGHBORHOOD MASKING - FROZEN) ##############################
ENCODER = {
    'type': 'TransformerEncoder',
    'source': 'pretrained',
    'ckpt_path': f'checkpoints/ContextContrastive_neighborhood_mask_3mo/xtraffic_SAN_BERNARDINO_30_{INPUT_LEN}_{OUTPUT_LEN}/*/Pretrain_NeighborhoodMasking_*.pt',
    'input_dim': 3,
    'd_model': D_MODEL,
...
CFG = EasyDict()
CFG.DESCRIPTION = 'Ablation: Frozen Encoder with Neighborhood Masking'
CFG.GPU_NUM = 1
...
############################## Model Configuration ##############################
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'STAEformer_NeighborhoodMask_Freeze'
...
CFG.TRAIN.CKPT_SAVE_DIR = os.path.join(
    'checkpoints',
    'ContextContrastive_freeze_neighborhood_mask_3mo',
