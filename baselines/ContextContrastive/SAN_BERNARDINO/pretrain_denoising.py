"""
Denoising Encoder Pre-training for SAN_BERNARDINO.

Trains the DenoisingEncoder to reconstruct clean traffic signals from
synthetically corrupted inputs. Uses 4 noise types (gaussian, bias, stuck, drift)
applied randomly during training.

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/pretrain_denoising.py', gpus='1')"
"""
import os
import sys
...
CFG = EasyDict()
CFG.DESCRIPTION = 'Denoising Encoder Pre-training (4 noise types, replace strategy)'
CFG.GPU_NUM = 1
...
############################## Model Configuration ##############################
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'DenoisingPretrain'
