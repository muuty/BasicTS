"""
Reconstruction-Only Pre-training (MAE-style) - Spatial 30%

No contrastive loss - only reconstruction loss.
Masks 30% of nodes and reconstructs them.

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/pretrain_reconstruction_spatial30.py', gpus='0')"
"""
import os
import sys
...
MODEL_PARAM = {
    ...
    'masking': {
        'spatial': {'enabled': True, 'ratio': MASK_RATIO},
...
CFG = EasyDict()
CFG.DESCRIPTION = f'Reconstruction-Only Pretrain: Spatial Masking {int(MASK_RATIO*100)}%'
CFG.GPU_NUM = 1
...
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = f'ContextContrastive_reconstruction_spatial{int(MASK_RATIO*100)}_3mo'
