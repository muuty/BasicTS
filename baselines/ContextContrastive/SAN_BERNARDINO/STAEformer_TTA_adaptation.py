"""
Test-Time Adaptation: Adaptation Mode

Self-supervised adaptation at test time using node masking reconstruction.
Adapts encoder per test sample, then predicts, then restores.

TTA Mode: adaptation
- ADAPT_STEPS: 1 gradient step per sample
- ADAPT_LR: 1e-4 (small to avoid overfitting)
- MASK_RATIO: 0.15 node masking

Expected: Better worst-k% performance (handles distribution shift)

Based on: spatial_mask freeze (best performing)

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/ContextContrastive/SAN_BERNARDINO/STAEformer_TTA_adaptation.py', gpus='0')"
"""
import os
import sys
...
TTA = {
    'ENABLED': True,
    'MODE': 'adaptation',         # Use adaptation TTA
    ...
    'ADAPT_LAYERS': 'all',        # Which layers to adapt: 'all', 'last', 'norm'
}

############################## General Configuration ##############################
CFG = EasyDict()
CFG.DESCRIPTION = 'TTA Adaptation: Spatial Mask Freeze + Self-supervised Adaptation at Test Time'
CFG.GPU_NUM = 1
...
############################## Model Configuration ##############################
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'STAEformer_TTA_Adaptation'
...
CFG.TRAIN.CKPT_SAVE_DIR = os.path.join(
    'checkpoints',
    'ContextContrastive_TTA_adaptation_3mo',
