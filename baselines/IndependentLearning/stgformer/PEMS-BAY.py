#!/usr/bin/env python3
"""
Independent Learning with STGformer on PEMS-BAY
...
try:
    ...
    ADJ_MATRIX = torch.tensor(adj_mx[0] if isinstance(adj_mx, list) else adj_mx, dtype=torch.float32)
except Exception:
    ADJ_MATRIX = None
    print(f"Warning: Could not load adjacency matrix for {DATA_NAME}")

# 노드 분할 및 subgraph_adj_list 생성 (HierSplit 방식)
...
# STGformer 베이스 모델 파라미터 (num_nodes, supports 제외 - wrapper에서 subgraph_adj_list로 자동 생성됨)
BASE_MODEL_PARAMS = {
    "in_steps": INPUT_LEN,
    ...
    "supports": None,  # subgraph_adj_list에서 자동 생성됨
    "num_heads": 4,
    "num_layers": 1,
    "dropout": 0.1,
    "mlp_ratio": 2.0,
...
MODEL_PARAM = {
    ...
    "subgraph_adj_list": subgraph_adj_list,  # HierSplit 방식으로 생성
}
...
CFG = EasyDict()
CFG.DESCRIPTION = f'Independent Learning with STGformer ({NUM_CLIENTS} clients)'
CFG.GPU_NUM = 1
...
############################## Model Configuration ##############################
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = f'IndependentLearning_{STGformer.__name__}_{NUM_CLIENTS}clients'
...
CFG.TRAIN.CKPT_SAVE_DIR = os.path.join(
    'checkpoints',
    f'IndependentLearning_{STGformer.__name__}',
