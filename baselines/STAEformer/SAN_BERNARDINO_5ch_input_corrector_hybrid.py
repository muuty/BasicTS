"""
STAEformer 5ch with Input Spillover Corrector — Hybrid Correction (Ablation).

Ablation: correction_mode='hybrid'
  - Reliability + cross-attention: per-node (pooled, like v1)
  - Correction delta: per-timestep (like full TV)

Compares with:
  - Node (identity config): all per-node
  - Full (tv config): all per-timestep

+ Identity regularization (λ=0.1) + noise augmentation.
50 epochs, CosineAnnealingLR.

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/STAEformer/SAN_BERNARDINO_5ch_input_corrector_hybrid.py', gpus='1')"
"""
import os
import sys
...
CFG = EasyDict()
CFG.DESCRIPTION = 'STAEformer 5ch + InputSpilloverCorrector (hybrid: node reliability, timestep correction)'
CFG.GPU_NUM = 1
...
############################## Model Configuration ##############################
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'STAEformer_5ch_input_corrector_hybrid'
