"""
STAEformer 5ch + RGCA (Reliability-Gated Cross-Attention), Time-Varying Correction.
Dataset: SAN_BERNARDINO_NODEAD (773 nodes, dead sensors excluded).

- unmasked_mae for training loss and evaluation
- Identity regularization (λ=0.1) + noise augmentation
- 50 epochs, CosineAnnealingLR

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/STAEformer/SAN_BERNARDINO_5ch_rgca_tv_nodead.py', gpus='1')"
"""
import os
import sys
import torch
...
CFG = EasyDict()
CFG.DESCRIPTION = 'STAEformer 5ch + RGCA tv + unmasked_mae, dead sensors excluded (773 nodes)'
CFG.GPU_NUM = 1
...
CFG.ENCODER = {
    ...
    'freeze': False,
    'lr': 1e-4,
    'ckpt_path': 'checkpoints/InputCorrectorPretrain_NODEAD/SAN_BERNARDINO_NODEAD_30_12_12/*/InputCorrectorPretrain_NODEAD_best_val_MAE.pt',
    'input_dim': 5,
    'd_model': 5,
    'hidden_dim': 64,
    'n_heads': 4,
...
############################## Model Configuration ##############################
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'STAEformer_5ch_rgca_tv_nodead'
