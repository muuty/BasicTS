"""
STAEformer 5ch with Noise-Invariant Encoder + Noise Augmentation.

Encoder pre-trained with noise-invariant objective:
  L = L_recon + L_pass + α * L_align
  L_align = MSE(encoder(x_noisy), encoder(x_clean))

Forces encoder to produce same representation for clean/noisy input.
Frozen pretrained encoder cleans input + training-time noise augmentation.

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/STAEformer/SAN_BERNARDINO_5ch_noise_invariant_noisy.py', gpus='1')"
"""
import os
import sys
...
CFG = EasyDict()
CFG.DESCRIPTION = 'STAEformer 5ch + Noise-Invariant Encoder (denoising+alignment) + Noise Augmentation'
CFG.GPU_NUM = 1
...
CFG.ENCODER = {
    ...
    'source': 'pretrained',
    'freeze': True,
    'ckpt_path': 'checkpoints/NoiseInvariantPretrain/SAN_BERNARDINO_30_12_12/d11c97c3c6cbcf69fc388a1616e098c2/NoiseInvariantPretrain_best_val_MAE.pt',
    'input_dim': 5,
    'd_model': 5,
    'hidden_dim': 64,
    'temporal_layers': 4,
...
############################## Model Configuration ##############################
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'STAEformer_5ch_noise_invariant_noisy'
