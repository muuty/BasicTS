"""
STAEformer 5ch with Input Spillover Corrector + Identity Regularization.

v1 corrector (unfrozen) + noise augmentation + identity constraint:
  - L = L_pred + λ * L_passthrough
  - L_passthrough = ||encoder(clean) - clean|| on physical channels
  - Forces encoder to be identity on clean inputs (CycleGAN identity loss principle)

No curriculum — identity loss directly preserves clean performance.
50 epochs, CosineAnnealingLR, λ=10.0.

Usage:
    python -c "from basicts import launch_training; launch_training('baselines/STAEformer/SAN_BERNARDINO_5ch_input_corrector_identity.py', gpus='1')"
"""
import os
import sys
...
CFG = EasyDict()
CFG.DESCRIPTION = 'STAEformer 5ch + InputSpilloverCorrector (unfrozen) + noise + identity loss'
CFG.GPU_NUM = 1
...
############################## Model Configuration ##############################
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = 'STAEformer_5ch_input_corrector_identity'
