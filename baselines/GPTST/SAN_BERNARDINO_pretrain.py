"""
GPT-ST Pre-training Configuration for SAN_BERNARDINO Dataset.

This configuration sets up GPT-ST for pre-training with:
- Masked reconstruction task
- Adaptive masking with curriculum learning
- STHCN encoder-decoder architecture
"""
...
from baselines.GPTST.runner import GPTSTPretrainRunner
from baselines.GPTST.loss import gptst_pretrain_loss
...
MODEL_PARAM = {
    ...
    "output_dim": 1,
    "input_base_dim": 1,  # Only flow for reconstruction
    ...
    "ada_mask_ratio": 1.0,
    "ada_type": "all",
    "change_epoch": 10,  # Switch to adaptive masking after this epoch
    "epochs": 30,
    "scaler_zeros": 0.0,
}
...
CFG = EasyDict()
CFG.DESCRIPTION = 'GPT-ST Pre-training on SAN_BERNARDINO'
CFG.GPU_NUM = 1

# Runner
CFG.RUNNER = GPTSTPretrainRunner
...
CFG.TRAIN.CKPT_SAVE_DIR = os.path.join(
    'checkpoints',
    MODEL_ARCH.__name__,
    '_'.join([DATA_NAME, str(CFG.TRAIN.NUM_EPOCHS), str(INPUT_LEN), str(OUTPUT_LEN), 'pretrain'])
)

# Loss function
CFG.TRAIN.LOSS = gptst_pretrain_loss
CFG.TRAIN.LOSS_ARGS = {
    'recon_weight': 1.0,
    'class_weight': 0.1,
    'null_val': NULL_VAL,
