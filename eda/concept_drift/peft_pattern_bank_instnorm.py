"""Instance Norm + Pattern Bank Adapter for cross-year adaptation.

Key question: Does Pattern Bank capture real pattern drift, or just scale drift?

If InstNorm + PB > InstNorm alone → PB captures real pattern drift
If InstNorm + PB ≈ InstNorm alone → PB was only doing scale correction

Compares:
  (a) No adaptation (baseline)
  (b) Instance Norm only (zero-shot, no target data)
  (c) PB only (from previous results)
  (d) Instance Norm + PB (fine-tune PB with instance norm applied)
  (e) Old PEFT embedding (from previous results)
"""
import sys
import os
import json
...
def apply_instance_norm(bx):
    """Apply per-sample instance normalization to flow channel (ch0).

    Args:
        bx: (B, T, N, C) tensor, already z-score normalized

    Returns:
        bx_normed: instance-normalized tensor
        inst_mean: (B, 1, N) per-sample mean for denormalization
        inst_std: (B, 1, N) per-sample std for denormalization
    """
    flow = bx[:, :, :, 0]  # (B, T, N)
    inst_mean = flow.mean(dim=1, keepdim=True)  # (B, 1, N)
    inst_std = flow.std(dim=1, keepdim=True) + 1e-5  # (B, 1, N)
    bx_normed = bx.clone()
...
def denorm_instance_norm(pred, inst_mean, inst_std):
    """Denormalize prediction from instance norm space.

    Args:
        pred: (B, T_out, N, 1) model output
        inst_mean: (B, 1, N) from apply_instance_norm
        inst_std: (B, 1, N) from apply_instance_norm

    Returns:
        pred_denormed: (B, T_out, N, 1) in z-score space
    """
...
def evaluate_with_instnorm(model, test_x, test_y, mean, std, stable_indices=None, batch_size=64):
    """Evaluate with instance norm applied at inference."""
    model.eval()
...
def finetune_adapter_with_instnorm(model, train_x, train_y, mean, std, epochs=10, lr=0.001):
    """Fine-tune PB adapter with instance norm applied during training."""
...
def main():
    ...
    print("=" * 70)
    print("INSTANCE NORM + PATTERN BANK ADAPTER")
    ...
    for source_year in years:
        ...
        for target_year in years:
            ...
            # (a) Instance Norm only (zero-shot, no PB, no fine-tuning)
            print("\n  [Instance Norm Only]")
            ...
            results[f"instnorm_only_{source_year}_{target_year}"] = res
            ...
            for n_days in FINETUNE_DAYS:
                ...
                # (b) Instance Norm + PB (both pattern_bank + node_weights)
                print(f"\n  [InstNorm + PB, {n_days}d]")
                model = load_model_with_adapter(ckpt_path)
                ...
                res = evaluate_with_instnorm(model, target_cache["test_x"], target_cache["test_y"],
                                             source_cache["mean"], source_cache["std"], stable_indices)
                results[f"instnorm_pb_{n_days}d_{source_year}_{target_year}"] = res
                print(f"    MAE={res['MAE']:.4f}, stable={res.get('stable_MAE','N/A'):.4f}")
                del model; torch.cuda.empty_cache()
    ...
    if os.path.exists("eda/concept_drift/cross_year_all_methods_results.json"):
        with open("eda/concept_drift/cross_year_all_methods_results.json") as f:
            instnorm_results = json.load(f)

    print(f"\n{'Method':<35} {'Params':>12} {'Data':>6} {'Avg MAE':>10} {'Avg Stable':>12}")
    print("-" * 77)

    methods = [
        ("No Adaptation", "no_adapt", "-", "0", prev_pb),
        ("Instance Norm (zero-shot)", "instnorm_only", "0", "0", results),
        ("PB Only 1d", "pb_both_1d", "7.3K(1.6%)", "1d", prev_pb),
        ("PB Only 3d", "pb_both_3d", "7.3K(1.6%)", "3d", prev_pb),
        ("PB Only 7d", "pb_both_7d", "7.3K(1.6%)", "7d", prev_pb),
        ("InstNorm + PB 1d", "instnorm_pb_1d", "7.3K(1.6%)", "1d", results),
        ("InstNorm + PB 3d", "instnorm_pb_3d", "7.3K(1.6%)", "3d", results),
        ("InstNorm + PB 7d", "instnorm_pb_7d", "7.3K(1.6%)", "7d", results),
        ("Old PEFT Emb 1d", "peft_emb_1d", "257K(56.8%)", "1d", prev_peft),
        ("Old PEFT Emb 3d", "peft_emb_3d", "257K(56.8%)", "3d", prev_peft),
        ("Old PEFT Emb 7d", "peft_emb_7d", "257K(56.8%)", "7d", prev_peft),
    ]
    ...
    if self_maes:
        print(f"{'Self-Year (oracle)':<35} {'--':>12} {'--':>6} {np.mean(self_maes):>10.4f}")

    # === Per-pair table ===
    print(f"\n{'='*70}")
    print("PER-PAIR: InstNorm+PB vs PB Only vs InstNorm Only")
    print(f"{'='*70}")
    hdr = "Train->Test"
    print(f"\n{hdr:<14} {'NoAdapt':>9} {'InstNorm':>9} {'PB 1d':>9} {'PB 3d':>9} {'PB 7d':>9} {'IN+PB1d':>9} {'IN+PB3d':>9} {'IN+PB7d':>9}")
    print("-" * 95)
    ...
    for s in years:
        for t in years:
            ...
            instn = results.get(f"instnorm_only_{s}_{t}", {}).get("MAE", float('nan'))
            pb1 = prev_pb.get(f"pb_both_1d_{s}_{t}", {}).get("MAE", float('nan'))
            pb3 = prev_pb.get(f"pb_both_3d_{s}_{t}", {}).get("MAE", float('nan'))
            pb7 = prev_pb.get(f"pb_both_7d_{s}_{t}", {}).get("MAE", float('nan'))
            ip1 = results.get(f"instnorm_pb_1d_{s}_{t}", {}).get("MAE", float('nan'))
            ip3 = results.get(f"instnorm_pb_3d_{s}_{t}", {}).get("MAE", float('nan'))
            ip7 = results.get(f"instnorm_pb_7d_{s}_{t}", {}).get("MAE", float('nan'))
            print(f"{row:<14} {no_a:>9.4f} {instn:>9.4f} {pb1:>9.4f} {pb3:>9.4f} {pb7:>9.4f} {ip1:>9.4f} {ip3:>9.4f} {ip7:>9.4f}")

    # Save
    output_path = "eda/concept_drift/peft_instnorm_pb_results.json"
