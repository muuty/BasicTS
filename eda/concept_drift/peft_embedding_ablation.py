"""Embedding ablation: Why Pattern Bank's low-rank constraint matters.

Tests:
1. PB (ours): softmax(W)@P, 7,336 params — prototype-constrained
2. Embedding-only: fine-tune adaptive_embedding directly, 21,432 params — unconstrained
3. Random embedding: replace learned embedding with random, 0 adaptation — proves spatial memory
4. Random embedding + PB: random init + PB adaptation — can PB recover?

All methods use RevIN (Instance Norm) trained backbone.
"""
import sys
import os
import json
...
class STAEformerWithPB(nn.Module):
    """Pattern Bank adapter (our method)."""
...
class STAEformerEmbeddingOnly(nn.Module):
    """Fine-tune adaptive_embedding directly (N*d params, unconstrained)."""

    def __init__(self, backbone):
        super().__init__()
        self.backbone = backbone
        # Delta is (N, d) — same shape as PB output, broadcast across timesteps
...
def finetune(model, trainable_names, train_x, train_y, mean, std,
...
def run_pb(ckpt_path, src, tgt, ft_x, ft_y):
    """Pattern Bank (our method)."""
    ...
    model, n_params = finetune(model, ["pattern_bank", "node_weights"],
...
def run_emb_only(ckpt_path, src, tgt, ft_x, ft_y):
    """Embedding-only fine-tune (unconstrained, N*d params)."""
    backbone = load_backbone(ckpt_path)
    model = STAEformerEmbeddingOnly(backbone)
    model, n_params = finetune(model, ["adp_emb_delta"],
                                ft_x, ft_y, src["mean"], src["std"])
    mae = evaluate(model, tgt["test_x"], tgt["test_y"], src["mean"], src["std"])
    del model, backbone; torch.cuda.empty_cache()
    return mae, n_params
...
def run_random_emb(ckpt_path, src, tgt):
    """Random embedding, no adaptation (proves spatial memory value)."""
    backbone = load_backbone(ckpt_path)
    # Replace learned embedding with random
    with torch.no_grad():
        nn.init.normal_(backbone.encoder.adaptive_embedding, mean=0.0, std=0.02)
...
def run_random_emb_pb(ckpt_path, src, tgt, ft_x, ft_y):
    """Random embedding + PB adaptation (can PB recover?)."""
    backbone = load_backbone(ckpt_path)
    with torch.no_grad():
        nn.init.normal_(backbone.encoder.adaptive_embedding, mean=0.0, std=0.02)
    model = STAEformerWithPB(backbone, MODEL_PARAM["num_nodes"],
...
def main():
    ...
    print("=" * 70)
    print("EMBEDDING ABLATION: Why Pattern Bank's low-rank constraint matters")
    print("=" * 70)
    print(f"Methods: PB(K={K}), Embedding-only(N*d), Random-emb, Random-emb+PB")
    ...
    for source_year, target_year in pairs:
        ...
        # (0) RevIN only (zero-shot, no adaptation)
        backbone = load_backbone(ckpt_path)
        backbone = backbone.to(DEVICE)
        ...
        # (0b) Random embedding (no adaptation)
        mae_rand = run_random_emb(ckpt_path, src, tgt)
        results[f"random_emb_{pair}"] = mae_rand
        print(f"  Random emb: {mae_rand:.2f}  (delta from RevIN: {mae_rand - mae_revin:+.2f})")
        ...
        for hours in FINETUNE_HOURS:
            ...
            # (1) PB (ours)
            mae_pb, np_pb = run_pb(ckpt_path, src, tgt, ft_x, ft_y)
            results[f"pb_{h_label}_{pair}"] = mae_pb
            print(f"    PB(K={K}):        {mae_pb:.2f}  ({np_pb:,} params)")

            # (2) Embedding-only
            mae_emb, np_emb = run_emb_only(ckpt_path, src, tgt, ft_x, ft_y)
            results[f"emb_only_{h_label}_{pair}"] = mae_emb
            print(f"    Emb-only:        {mae_emb:.2f}  ({np_emb:,} params)")

            # (3) Random emb + PB
            mae_rpb, np_rpb = run_random_emb_pb(ckpt_path, src, tgt, ft_x, ft_y)
            results[f"random_pb_{h_label}_{pair}"] = mae_rpb
            print(f"    Random+PB:       {mae_rpb:.2f}  ({np_rpb:,} params)")

            # Comparison
            diff_emb = mae_emb - mae_pb
            diff_rpb = mae_rpb - mae_pb
            print(f"    Emb-PB gap: {diff_emb:+.2f}  Random+PB-PB gap: {diff_rpb:+.2f}")
    ...
    # RevIN and Random embedding baselines
    revin_maes = [results[f"revin_{s}_{t}"] for s, t in pairs if f"revin_{s}_{t}" in results]
    rand_maes = [results[f"random_emb_{s}_{t}"] for s, t in pairs if f"random_emb_{s}_{t}" in results]
    print(f"\n  RevIN only (zero-shot):       {np.mean(revin_maes):.2f}")
    print(f"  Random embedding (zero-shot): {np.mean(rand_maes):.2f}")
    print(f"  -> Embedding spatial memory value: {np.mean(rand_maes) - np.mean(revin_maes):+.2f} MAE")

    # Per-hour comparison
    print(f"\n{'':>8} {'PB':>8} {'Emb-only':>10} {'Rand+PB':>10} │ {'Emb-PB':>8} {'Rand-PB':>9}")
    print("-" * 60)
    for hours in FINETUNE_HOURS:
        h_label = f"{hours}h" if hours < 24 else f"{hours//24}d"
        pb_maes = [results.get(f"pb_{h_label}_{s}_{t}", float('nan')) for s, t in pairs]
        emb_maes = [results.get(f"emb_only_{h_label}_{s}_{t}", float('nan')) for s, t in pairs]
        rpb_maes = [results.get(f"random_pb_{h_label}_{s}_{t}", float('nan')) for s, t in pairs]

        pb_avg = np.nanmean(pb_maes)
        emb_avg = np.nanmean(emb_maes)
        rpb_avg = np.nanmean(rpb_maes)

        print(f"{h_label:>8} {pb_avg:>8.2f} {emb_avg:>10.2f} {rpb_avg:>10.2f} │ "
              f"{emb_avg - pb_avg:>+8.2f} {rpb_avg - pb_avg:>+9.2f}")

    # Win rate
    print(f"\n{'PB win rate vs Emb-only (out of 6)':>40}")
    print("-" * 50)
    for hours in FINETUNE_HOURS:
        h_label = f"{hours}h" if hours < 24 else f"{hours//24}d"
        wins = 0
        for s, t in pairs:
            pb = results.get(f"pb_{h_label}_{s}_{t}", float('nan'))
            emb = results.get(f"emb_only_{h_label}_{s}_{t}", float('nan'))
            if pb < emb:
                wins += 1
        print(f"  {h_label:>4}: {wins}/6")

    # Per-pair detail table
    print(f"\n{'='*70}")
    print("PER-PAIR DETAIL")
    print(f"{'='*70}")
    print(f"\n{'Pair':<12} {'RevIN':>7} {'RandE':>7} │", end="")
    for hours in FINETUNE_HOURS:
        h = f"{hours}h" if hours < 24 else f"{hours//24}d"
        print(f" PB{h:>3} Emb{h:>3}", end="")
    print()
    print("-" * (30 + 14 * len(FINETUNE_HOURS)))

    for s, t in pairs:
        pair = f"{s}_{t}"
        rev = results.get(f"revin_{pair}", float('nan'))
        rnd = results.get(f"random_emb_{pair}", float('nan'))
        print(f"{s}->{t:<5} {rev:>7.2f} {rnd:>7.2f} │", end="")
        for hours in FINETUNE_HOURS:
            h_label = f"{hours}h" if hours < 24 else f"{hours//24}d"
            pb = results.get(f"pb_{h_label}_{pair}", float('nan'))
            emb = results.get(f"emb_only_{h_label}_{pair}", float('nan'))
            print(f" {pb:>6.2f} {emb:>6.2f}", end="")
        print()

    output_path = "eda/concept_drift/peft_embedding_ablation_results.json"
